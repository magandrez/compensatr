...
# @return [TrueClass, FalseClass] true if it meets the minimum, false otherwise
...
if $PROGRAM_NAME == __FILE__ # Let the script run unless Rspec is the caller
  ...
  1.upto(MAX_ITERATIONS) do |_i|
    ...
    puts "Money spent in new best selection of projects (in money units):\
           #{money_spent}"
    puts "Count of different project locations by continent:\
         #{count_continents(best_selection)}"
    expenditures = add_costs(best_selection)
    puts 'Representation per group: '
    puts "  Short term value (in money units): #{expenditures[:short]}.\
         In percentage #{((expenditures[:short].to_f /
money_spent) * 100).round(2)}%."
    puts "  Medium term value (in money units): #{expenditures[:medium]}.\
 In percentage #{((expenditures[:medium].to_f / money_spent) * 100).round(2)}%"
    puts "  Long term value: (in money units) #{expenditures[:long]}.\
 In percentage #{((expenditures[:long].to_f / money_spent) * 100).round(2)}%"
    puts '++++++++++++'
  end
  puts " ------------ \n Best selection: \n #{best_selection}"
